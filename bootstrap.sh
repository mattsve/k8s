...
kubectl create secret generic op-credentials --from-file=1password-credentials.json --from-literal=token=$(op create connect token k8s op-k8s-operator --vault k8s,r) --namespace=onepassword --dry-run=client -o yaml | kubectl apply -f -
